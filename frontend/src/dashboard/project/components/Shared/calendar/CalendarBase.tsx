...
import {
  ...
  DOT_STROKE,
  UNIT_OPTIONS,
  WRAPPER_INTERACTIVE_SELECTOR,
  INTERACTIVE_SAFE_PAD,
} from "./constants";
import { useDayOverlay } from "./hooks";
import { formatDateLabel, getDateKey, safeParse } from "./utils";
...
function computeFinalCost(qty: number | string, budget: number | string, mark: number | string): string {
...
const CalendarBase: React.FC<CalendarBaseProps> = ({
  ...
  const overlayHeaderId = useMemo(
    () => (overlayDayKey ? `${dayHeaderIdPrefix}-${overlayDayKey}` : dayHeaderIdPrefix),
    [dayHeaderIdPrefix, overlayDayKey]
  );
  ...
  const openAddEventModal = useCallback(
    (e?: ReactMouseEvent<HTMLButtonElement>, dateKeyParam?: string) => {
      e?.stopPropagation();
      e?.preventDefault();
  ...
  return (
    <div
      className={`dashboard-item ${wrapperClassName}${wrapperHover ? " calendar-card-hover" : ""}`}
